...
public class BrowserLauncher {
	...
	private static Object locateBrowser() {
		...
		case MRJ_2_0:
			try {
				...
				// call
				// locateBrowser(), we'e'll already have
		...
		case MRJ_2_1:
			...
			for (int i = 0; i < systemFolderFiles.length; i++) {
				try {
					...
					// We' 'MACS' and
					// a type of 'R'. Only requiring the type results in
					// non-Finder
	...
	public static void openURL(String url) throws IOException {
		...
		case OTHER:
			...
			// Netscape
			/*
			 * process = Runtime.getRuntime().exec(new String[] { (String)
			 * browser, NETSCAPE_REMOTE_PARAMETER, NETSCAPE_OPEN_PARAMETER_START +
			 * url + NETSCAPE_OPEN_PARAMETER_END }); try { int exitCode =
			 * process.waitFor(); if (exitCode != 0) { // if Netscape was not
			 * open Runtime.getRuntime().exec(new String[] { (String) browser,
			 * url }); } } catch (InterruptedException ie) { throw new
			 * IOException("InterruptedException while launching browser: " +
			 * ie.getMessage()); }
			 */
			throw new IOException("Browser Launch buggy in Linux");
			// break;
		default:
